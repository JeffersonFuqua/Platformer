...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller {
		...
		protected PlatformerActions pActions;
        //Jump key variables;
		...
		protected virtual Vector3 CalculateMovementDirection()
		{
			...
			{
				_velocity += tr.right * pActions.PlayerControls.Movement.ReadValue<Vector2>().x;
				_velocity += tr.forward * pActions.PlayerControls.Movement.ReadValue<Vector2>().y;
			}
			else
			{
				//If a camera transform has been assigned, use the assigned transform's axes for movement direction;
				//Project movement direction so movement stays parallel to the ground;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized * pActions.PlayerControls.Movement.ReadValue<Vector2>().x;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized * pActions.PlayerControls.Movement.ReadValue<Vector2>().y;
			}
		...
		protected virtual bool IsJumpKeyPressed()
		{
			...
			return pActions.PlayerControls.Jump.ReadValue<float>() > 0;//characterInput.IsJumpKeyPressed();
		}
