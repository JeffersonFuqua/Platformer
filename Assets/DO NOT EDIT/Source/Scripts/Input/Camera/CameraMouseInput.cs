...
namespace CMF
{
    ...
    public class CameraMouseInput : CameraInput
    {
        ...
        private PlatformerActions pActions;
        ...
        public override float GetHorizontalCameraInput()
        {
            float _input;
            //Get raw mouse input;
            if(pActions.PlayerControls.MouseLook.ReadValue<Vector2>().x != 0)
                _input = pActions.PlayerControls.MouseLook.ReadValue<Vector2>().x * Time.deltaTime;
            else
               _input = pActions.PlayerControls.ControllerLook.ReadValue<Vector2>().x;//Input.GetAxisRaw(mouseHorizontalAxis);
        ...
        public override float GetVerticalCameraInput()
        {
            float _input;
            //Get raw mouse input;
            if (pActions.PlayerControls.MouseLook.ReadValue<Vector2>().y != 0)
                _input = pActions.PlayerControls.MouseLook.ReadValue<Vector2>().y * Time.deltaTime;
            else
                _input = pActions.PlayerControls.ControllerLook.ReadValue<Vector2>().y;//Input.GetAxisRaw(mouseHorizontalAxis);
